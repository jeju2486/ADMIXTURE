...
rm(list=ls())
gname = "HGDP.hg19"
rdwd = "C:\\ADMIXTURE\\ADMIXTURE_result\\"
setwd("C:\\ADMIXTURE")
```
...
print(paste("#people in total population :",length(tpopvec),sep=" "))
print(paste("#type of population :",nrow(popinfo),sep=" "))
```
...
print(paste("blank rows betwwen", blank.nums,blank.nums+1,sep=" "))
```
```{r}
...
print(paste("K max is",Kmax, sep=" "))
print(paste("blank columns between", empty.cnums,empty.cnums+1,sep=" "))
```
...
for (K in 3:Kmax) {
  ...
  ##Active below code to insert information of individuals
  ##text(x=lgxv, y=lgyv-0.1, label=lgv, srt=90, cex=0.5, adj=c(1,0.5), font=2, col=lg.cv)
  text(x=lgxv*0.5, y=lgyv+1.1, label=cpopvec, srt=90, cex=0.5, adj=c(0,0.5), font=4, col=lg.cv2)
}
dev.off()
```
